...
public class MysqlSqlStdOperatorSupport extends MysqlSqlDialect implements SqlStdOperatorSupport, FetchAndOffsetSupport {
    ...
    static ConcurrentSkipListSet<StdSqlOperator> OWN_SUPPORTED = new ConcurrentSkipListSet<>(
            ...
            IF, COALESCE, AGG_DATE_YEAR, AGG_DATE_QUARTER, AGG_DATE_MONTH, AGG_DATE_WEEK, AGG_DATE_DAY, AGG_DATE_HOUR, AGG_DATE_MINUTE));
    ...
    public boolean unparseStdSqlOperator(SqlWriter writer, SqlCall call, int leftPrec, int rightPrec) {
        ...
        switch (operator) {
            ...
            case AGG_DATE_DAY:
                ...
                return true;
            case AGG_DATE_HOUR:
                writer.print("DATE_FORMAT(" + call.getOperandList().get(0).toString() + ",'%Y-%m-%d %h')");
                return true;
            case AGG_DATE_MINUTE:
                writer.print("CONCAT(left(DATE_FORMAT(" + call.getOperandList().get(0).toString() + ",'%Y-%m-%d %h:%i'),15),'0')");
                return true;
            default:
                break;
        }
