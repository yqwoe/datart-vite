...
public class JdbcDataProvider extends DataProvider {
    ...
    public Set<TableInfo> readTableInfos(DataProviderSource source, String database) {
        try {
            JdbcDataProviderAdapter adapter = matchProviderAdapter(source);
            return adapter.readAllTableInfos(database);
    ...
    public Set<Column> readTableColumnsBySql(DataProviderSource source, String sql) {
        try {
            JdbcDataProviderAdapter adapter = matchProviderAdapter(source);
            return adapter.readTableColumnsBySql(sql);
        } catch (SQLException e) {
            throw new DataProviderException(e);
        }
    }
    ...
    public boolean createTable(DataProviderSource target, String table,Set<Column> columns) {
        try {
            ...
            st.add("db", args[0]);
            st.add("tableName", args[1]);
            st.add("fields", columns);
            st.add("primaryKeys", columns.stream().filter(c->c.isPrimaryKey()).map(c->c.getName()).collect(Collectors.toList()));
            String sql = st.render();
            JdbcDataProviderAdapter adapter = matchProviderAdapter(target);
            return adapter.executeCreateSql(sql);
        } catch (SQLException e) {
            throw new DataProviderException(e);
        }
    }
    ...
    public Set<Column> readTableColumnInfos(DataProviderSource source, String table) {
        try {
            JdbcDataProviderAdapter adapter = matchProviderAdapter(source);
            return adapter.readTableColumnInfo(table);
        } catch (SQLException e) {
            throw new DataProviderException(e);
        }
    }
    ...
    public boolean callFunction(DataProviderSource source, String function) {
        try {
            JdbcDataProviderAdapter adapter = matchProviderAdapter(source);
            return adapter.callFunction(function);
        } catch (SQLException e) {
            throw new DataProviderException(e);
        }
    }
        ...
        for (DataProviderConfigTemplate.Attribute attribute : configTemplate.getAttributes()) {
            ...
            if (attribute.getName().equals("dbType")) {
                ...
                List<Object> dbInfos = jdbcDriverInfos.stream().map(info -> {
                    ...
                    properties.setProperty(DB, info.getDb()  == null ? "" : info.getDb());
                    ...
                    properties.setProperty(EXTRACT, info.getExtract() == null ? "" : info.getExtract());
                    return properties;
...
//    private void parseColumns(List<Map<>>){
//
//    }
}
