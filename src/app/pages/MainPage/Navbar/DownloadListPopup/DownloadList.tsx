...
const DownloadFileItem: FC<DownloadFileItemProps> = ({
  ...
  const { name, status, updateTime, createTime } = restProps;
  ...
  const handleDownloadFile = () => {
    return status === DownloadTaskState.DONE ||
      status === DownloadTaskState.DOWNLOADED
      ? onDownloadFile(restProps)
      : null;
  };
  return (
    <DownloadFileItemWrapper>
      <span className={titleClasses} onClick={handleDownloadFile}>
        {name}
      </span>
