...
const ChartPreviewBoard: FC<{
  ...
  ({
    ...
    useEffect(() => {
      const jumpFilterParams: PendingChartDataRequestFilter[] = filterSearchUrl
        ? parse(filterSearchUrl, { ignoreQueryPrefix: true })?.filters
        : undefined;
      const jumpVariableParams: Record<string, any[]> = filterSearchUrl
        ? parse(filterSearchUrl, {
    ...
    const handleAddToDashBoard = useCallback(
      (dashboardId, dashboardType) => {
        ...
        try {
          history(`/organizations/${orgId}/vizs/${dashboardId}/boardEditor`, {
            state: {
    ...
    return (
      <StyledChartPreviewBoard>
        ...
        <div key="viewDetailPanelContextHolder">
          {viewDetailPanelContextHolder}
        </div>
        <div key="jumpDialogContextHolder">{jumpDialogContextHolder}</div>
        <div key="openJumpVizDialogModalContextHolder">
...
const ChartWrapper = styled.div<{ theme: DefaultTheme }>`
  position: relative;
  display: flex;
  flex: 1;
  height: 100%;
...
const StyledChartDrillPathsContainer = styled.div<{ theme: DefaultTheme }>`
  background-color: ${p => p.theme.componentBackground};
`;
