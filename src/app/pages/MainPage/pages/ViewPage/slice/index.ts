...
import { Schema, Table, ViewState, ViewViewModel } from './types';
...
const slice = createSlice({
  ...
  initialState,
  reducers: {
    changeCurrentEditor(state, action: PayloadAction<string>) {
    ...
    addTables(
      ...
      if (database) {
        database.children = tables.map(table => ({
          key: [databaseName, table.name].join(String.fromCharCode(0)),
          title: table.name,
          type: 'table',
          comment: table.comment,
          value: [databaseName, table.name],
