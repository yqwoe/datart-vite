...
export const Toolbar = memo(
  ({ allowManage, allowEnableViz, type }: ToolbarProps) => {
    ...
    const filterDateViewNode = useCallback((inputValue, node) => {
      return node?.children?.toLowerCase().includes(inputValue.toLowerCase());
    }, []);

    const current = useSelector(selectCurrentEditingView);
              ...
              <>
                {allowManage && (
                  <Select
                    ...
                    className="source"
                    showSearch
                    filterOption={filterDateViewNode}
                  >
                    {sources.map(({ id, name }) => (
