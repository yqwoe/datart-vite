import type { NsEdgeCmd, NsGraph, NsNodeCmd } from '@antv/xflow';
...
export namespace MockApi {
  ...
  /** 添加节点 */
  export const addNode: NsNodeCmd.AddNode.IArgs['createNodeService'] =
    async args => {
  ...
  /** 删除节点 */
  export const delNode: NsNodeCmd.DelNode.IArgs['deleteNodeService'] =
    async args => {
      const { nodeConfig } = args;
  ...
  /** 添加边 */
  export const addEdge: NsEdgeCmd.AddEdge.IArgs['createEdgeService'] =
    async args => {
  ...
  /** 删除边 */
  export const delEdge: NsEdgeCmd.DelEdge.IArgs['deleteEdgeService'] =
    async args => {
      const { edgeConfig } = args;
      const promise: Promise<boolean> = new Promise(resolve => {
        setTimeout(() => {
