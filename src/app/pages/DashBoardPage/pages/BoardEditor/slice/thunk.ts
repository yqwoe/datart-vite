...
>(
  ...
  async (
    ...
    dispatch(boardActions.setViewMap(viewViews));
    const widget = widgetManager.toolkit(ORIGINAL_TYPE_MAP.ownedChart).create({
      boardType: boardType,
      datachartId: chartId,
      relations: [],
      name: dataChart.name,
...
>(
  ...
  async (
    ...
    const widget = widgetManager.toolkit(originalType).create({
      boardType: boardType,
      datachartId: chartId,
      relations: [],
      content: dataChart,
...
>(
  ...
  async ({ widgetId, option }, { getState, dispatch, rejectWithValue }) => {
    ...
    const requestParams = getChartWidgetRequestParams({
      widgetId,
      widgetMap,
      viewMap,
      option,
