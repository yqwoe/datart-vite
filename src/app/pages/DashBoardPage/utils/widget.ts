...
export const createChartWidgetContent = (subType: WidgetContentChartType) => {
  const content: ChartWidgetContent = {
    type: subType,
  };
  return content;
};
...
export const getWidgetMap = (
  ...
  widgetList
    ...
    .forEach(widget => {
      const dataChart = (widget.config.content as any).dataChart as DataChart;
      if (dataChart) {
...
export const getValueByRowData = (
  ...
  const toCaseField = JSON.parse(fieldName).join('.');
...
export function cloneWidgets(args: {
  ...
  widgets.forEach(widget => {
    ...
    if (newWidget.config.type === 'chart') {
      const dataChart = dashboardDataChartMap[newWidget.datachartId];
...
export const convertToTree = (col, buildingMethod) => {
  ...
  const copyCol = CloneValueDeep(col);
  const emptyParentList = ['null', 'undefined', 'false'];
