...
const CalcAction: FC<{
  ...
  const t = useI18NPrefix(`viz.common.enum.calc`);

  const actionNeedNewRequest = true;
  const [calc, setCalc] = useState(config?.calc);

  const items: CalcFieldActionType[] = [];
  ...
  if (
    ...
    items.push(CalcFieldActionType.DateRatio);
  }

  return (
    <>
      <Menu.Item
        key={CalcFieldActionType.None}
        eventKey={CalcFieldActionType.None}
