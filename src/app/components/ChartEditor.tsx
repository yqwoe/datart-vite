...
export const ChartEditor: FC<ChartEditorProps> = ({
    ...
    () => {
      ...
      if (container === 'dataChart') {
        ...
        if (chartType === 'widgetChart') {
          ...
          if (!originChart) {
            dispatch(actions.updateChartAggregation(false));
          }
        } else {
          // chartType === 'dataChart'
          dispatch(
  ...
  const clearDataConfig = useCallback(() => {
    ...
    const targetChartConfig = CloneValueDeep(currentChart?.config);
      ...
      if (dataChartId) {
        ...
        try {
          addVizFn({
            ...
            callback: folder => {
              folder && history(`/organizations/${orgId}/vizs/${folder.relId}`);
            },
          });
        } catch (error) {
          errorHandle(error);
