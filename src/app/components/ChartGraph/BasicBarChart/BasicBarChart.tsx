...
class BasicBarChart extends Chart implements IChartLifecycle {
  ...
  getOptions(
    ...
    const columns = uniq(chartDataSet.map(row => row.getCell(groupConfigs[0])));

    const axisInfo = {
      xAxis: this.getXAxis(styleConfigs, xAxisColumns, columns.length),
  ...
  private getLegendStyle(styles, series): LegendStyle {
    ...
    switch (legendPos) {
      ...
      case 'left':
        orient = 'vertical';
        positions = { left: 8, top: 16, bottom: 24, width: 50 };
        break;
      case 'bottomLeft':
        orient = 'vertical';
        positions = { left: 8, bottom: 0, width: 50 };
        break;
      case 'bottomRight':
        orient = 'vertical';
        positions = { right: 8, bottom: 10, width: 50 };
        break;
      case 'right':
        orient = 'vertical';
        positions = { right: 8, top: 16, bottom: 24, width: 50 };
        break;
    }
  ...
  private getGraphicStyle(
    styles,
    chartDataSet,
    groupConfigs,
    aggregateConfigs,
  ) {
    const [graphic] = getStyles(styles, ['graphic'], ['graphic']);

    const [lineStyle] = getStyles(styles, ['xAxis'], ['lineStyle']);

    if (graphic) {
      const columns = uniq(
        chartDataSet.map(row => row.getCell(groupConfigs[0])),
      );
      ...
      return [
        ...
        {
          position: 'bottom', // 将分组x轴位置定至底部，不然默认在顶部
          offset: 33, // 向下偏移，使分组文字显示位置不与原x轴重叠
          max: columns.length - 1,
          interval: columns.length - 1,
          axisTick: {
            length: 30, // 延长刻度线做分组线
            inside: true, // 使刻度线相对轴线在上面与原x轴相接，默认在轴线下方
            lineStyle: { color: '#000' }, // 非必须，仅为了演示，明显标示出分组刻度线
        ...
        ...aggregateConfigs.map((agg, index) => ({
          position: 'bottom', // 将分组x轴位置定至底部，不然默认在顶部
          offset: 60 + index * 30, // 向下偏移，使分组文字显示位置不与原x轴重叠
          max: columns.length - 1,
          axisTick: {
            length: 30, // 延长刻度线做分组线
            inside: true, // 使刻度线相对轴线在上面与原x轴相接，默认在轴线下方
          ...
          data: chartDataSet.map(row => row.getCell(agg)),
        })),
      ];
    }
    return [];
  ...
  private getTooltipFormatterFunc(
    ...
    return seriesParams => {
      const params = Array.isArray(seriesParams)
        ? seriesParams
        : [seriesParams];
      const result = getSeriesTooltips4Rectangular2(
        chartDataSet,
        params[0],
        groupConfigs,
        colorConfigs,
