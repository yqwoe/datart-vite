...
class PivotSheetChart extends ReactChart {
  ...
  private hierarchyCollapse = true;
  private drillLevel = 0;
  ...
  getOptions(
    ...
    const baseSectionConfigRows: ChartDataSectionField[] = dataConfigs
      .filter(c => c.type === ChartDataSectionType.Group)
      .filter(c => c.key === 'base')
      .flatMap(config => config.rows || []);
    ...
    const [play, delay, pauseOnHover, direction, speed] = getStyles(
      styleConfigs,
      ['scroll'],
    ...
    return {
      ...
      dataCfg: {
        ...
        data: chartDataSet?.map(row => ({
          ...row.convertToObject(),
          isProgress: enableProgress,
          value: row.getCell(baseSectionConfigRows[0]),
        })) as Data[],
  ...
  private getRowAndColStyle(
    ...
    const [metricNameShowIn, enableProgress, device, enableExpandRow] =
      getStyles(
        style,
        ['style'],
        ['metricNameShowIn', 'enableProgress', 'device', 'enableExpandRow'],
      );
    return {
      colCfg: {
        hideMeasureColumn: enableProgress,
      ...
      //     : new DataCell(viewMeta, viewMeta.spreadsheet);
      // },
