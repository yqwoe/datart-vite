...
function LiquidChart({ dHelper }) {
  ...
  return {
    config: {
      ...
      i18ns: [
        ...
        },
        {
          lang: 'en',
          translation: {
            chartName: 'liquid Chart',
          },
        },
      ],
    },

    isISOContainer: 'liquid-chart',
    ...
    getOptions(dataset, config) {
      ...
      return {
        percent: Math.min(percent * 0.01),
        ...this.getOutLine(styleConfigs),
        ...this.getWave(styleConfigs),
        ...this.getStatistic(styleConfigs, aggregateConfigs),
        liquidStyle: {
          fill: color,
          opacity: opacity,
        },
    ...
    getStatistic(styles, aggregateConfigs) {
      const [title, prefix, suffix, max, font, titleFont] = dHelper.getStyles(
        styles,
        ['statistic'],
        ['title', 'prefix', 'suffix', 'max', 'font', 'titleFont'],
      );
      let statistic = {};
      if (title) {
        statistic.title = {
