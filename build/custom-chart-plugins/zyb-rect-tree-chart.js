...
function D3JSScatterChart({ dHelper }) {
  ...
  return {
    ...
    getOptions(dataset, config) {
      ...
      // 列表数据转树形数据
      const metricsLength = dataConfigs.filter(c => c.type === 'aggregate')[0]
        .rows?.length;
      let treeData = this.arrDataToTreeLevel(
        dataset.rows,
        metricsLength,
        dataset.columns,
        colorConfigs,
    ...
    getBreadCrumbStyle(style) {
      const [show, top, left, height, emptyItemWidth, font] = dHelper.getStyles(
        style,
        ['breadcrumb'],
        ['show', 'top', 'left', 'height', 'emptyItemWidth', 'font'],
      );
      return {
        show,
        top,
    ...
    getBorderStyle(style) {
      const [color, width] = dHelper.getStyles(
        style,
        ['border'],
        ['color', 'width'],
      );
    ...
    getLabelStyle(style) {
      const [showlabel, position, font] = dHelper.getStyles(
        style,
        ['label'],
    ...
    arrDataToTreeLevel(data, metricsLength, column, colorConfigs) {
      ...
      for (let i = 0; i <= datas?.length - 1; i++) {
        if (datas[i]?.length > metricsLength) {
          ...
          if (index == -1) {
            let color = colorConfigs
              ?.find(item => item.colName == column[data[i]?.length - 1].name)
              ?.color?.colors.find(i => i.key == nodeName);
      ...
      newArr.forEach(item => {
        if (item.children?.length) {
          item.children = this.arrDataToTreeLevel(
            item.children,
            metricsLength,
            column,
            colorConfigs,
