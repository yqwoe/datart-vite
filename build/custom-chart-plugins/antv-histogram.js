...
function AntVHistogramChart({ dHelper }) {
  ...
  return {
    ...
    dependency: [
      'https://cdn.jsdelivr.net/npm/@antv/g2@4.1.47/dist/g2.min.js',
    ...
    onUpdated(props, context) {
      ...
      const dimensionName =
        props.config.datas[0] &&
        props.config.datas[0].rows[0] &&
        props.config.datas[0].rows[0].colName
          ? props.config.datas[0].rows[0].colName
          : null;
      const metas = dimensionName
        ? [{ name: dimensionName }, { name: metricsName }]
        : [{ name: metricsName }];
      ...
      dv.transform({
        type: 'bin.histogram',
        field: metricsName,
        binWidth: dHelper.getStyles(styles, ['xAxis'], ['barWidth']),
      ...
      const chartIns = this.chart
        ...
        .color(
          props.config.datas[1].rows[0].color
            ? props.config.datas[1].rows[0].color.start
            : '#6395f9',
        );
