...
public class DownloadServiceImpl extends BaseService implements DownloadService {
    ...
    public Download submitDownloadTask(DownloadCreateParam downloadParams, String clientId) {
        ...
        if(null == user){
            Exceptions.msg("禁止未登录用户下载数据");
        }
        final String downloadUser = user.getUsername();
        TaskExecutor.submit(()->{
            log.debug("downloadUser is {}",downloadUser);
            try {
                ...
                try {
                    ...
                    log.debug("Download Task execute error {}",e.getMessage());
                    download.setStatus((byte) -1);
                }
