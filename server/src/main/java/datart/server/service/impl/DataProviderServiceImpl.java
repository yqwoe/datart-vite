...
public class DataProviderServiceImpl extends BaseService implements DataProviderService {
    ...
    private final  Map<String,String> defaultMaps = new HashMap<String,String>(){{
        put("string","VARCHAR(1024)");
        put("numeric","NUMERIC");
        put("date","timestamp");
    }};

    private final  Map<String,List<MetaInfo>> schemaMaps = new HashMap<String,List<MetaInfo>>();
    ...
    @Override
    public Set<TableInfo> readTableInfos(String sourceId, String database) {
        Source source = retrieve(sourceId, Source.class, false);
        return dataProviderManager.readTableInfos(parseDataProviderConfig(source), database);
    }
    ...
    @Override
    public Set<Column> readTableColumnInfos(String sourceId, String table) {
        Source source = retrieve(sourceId, Source.class, false);
        return dataProviderManager.readTableColumnInfos(parseDataProviderConfig(source), table);
    }

    @Override
    public Set<Column> readTableColumnsBySql(String sourceId, String sql) {
        Source source = retrieve(sourceId, Source.class, false);
        return dataProviderManager.readTableColumnsBySql(parseDataProviderConfig(source),sql);
    }
    ...
    @Override
    public boolean callFunction(String sourceId, String function) {
        Source source = retrieve(sourceId, Source.class, false);
        return dataProviderManager.callFunction(parseDataProviderConfig(source),function);
    }
