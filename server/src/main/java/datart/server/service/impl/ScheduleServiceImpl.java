...
import datart.server.job.DingtalkJob;
import datart.server.job.EmailJob;
import datart.server.job.ScheduleJob;
import datart.server.job.WeChatJob;
...
public class ScheduleServiceImpl extends BaseService implements ScheduleService {
    ...
    public boolean execute(String scheduleId) {
        ...
        ScheduleJob job;
        if (JobType.WECHAT.name().equalsIgnoreCase(schedule.getType())) {
            job = new WeChatJob();
        } else if (JobType.DINGTALK.name().equalsIgnoreCase(schedule.getType())) {
            job = new DingtalkJob();
        } else {
            job = new EmailJob();
        }
    ...
    @Override
    public boolean unarchive(String id, String newName, String parentId, Integer index) {
