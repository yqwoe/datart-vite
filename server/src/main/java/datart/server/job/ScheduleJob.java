...
public abstract class ScheduleJob implements Job, Closeable {
    ...
    protected final List<String> shareUrls = new LinkedList<>();
    ...
    public ScheduleJob() {
        ...
        env = Application.getBean(Environment.class);
    }
    ...
    public void doGetData() throws Exception {
        ...
        for (ScheduleJobConfig.VizContent vizContent : config.getVizContents()) {
            ...
            for (AttachmentType type : config.getAttachments()) {
                DownloadCreateParam param;
                if(AttachmentType.SHARE_URL.equals(type)){
                    shareUrls.add(getShareUrl(folder));
                }else{
                    param = setVizId(downloadCreateParam, folder, type);
    ...
    private String getShareUrl(Folder folder){
        ...
        shareCreateParam.setExpiryDate(DateUtils.addHours(new Date(), 24));
        shareCreateParam.setAuthenticationMode(ShareAuthenticationMode.CODE);
        ...
        return Application.getWebRootURL() + "/" + shareCreateParam.getVizType().getShareRoute() + "/" + share.getId() + "?type=" + share.getAuthenticationMode()+"&_k="+share.getAuthenticationCode();
    }
