    ...
    public boolean execute(String sourceId) throws Exception {
        ...
        if (CollectionUtils.isNotEmpty(databases)) {
            for (String database : databases) {
                ...
                Set<TableInfo> tables = dataProviderService.readTableInfos(sourceId, database);
                if (CollectionUtils.isNotEmpty(tables)) {
                    for (TableInfo table : tables) {
                        table.setColumns(dataProviderService.readTableColumns(sourceId, database, table.getName()));
