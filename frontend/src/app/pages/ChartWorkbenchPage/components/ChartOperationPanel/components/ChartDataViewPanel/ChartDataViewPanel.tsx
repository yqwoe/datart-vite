...
const ChartDataViewPanel: FC<{
  ...
  const dataviewTreeData = useSelector(state =>
    dataviewTreeSelector(state, getIcon, getSelectable),
  );
  ...
  const handleDataViewChange = useCallback(
    value => {
      ...
      const data = chartConfig?.datas?.filter(v => v.rows && v.rows.length);
  ...
  const editView = useCallback(() => {
    const orgId = dataView?.orgId as string;
    const viewId = dataView?.id as string;
    history(`/organizations/${orgId}/views/${viewId}`);
  ...
  useEffect(() => {
    buildAllMetaFields(dataView?.type === 'STRUCT', 'byOriginalFieldSort');
  ...
  return (
    <StyledChartDataViewPanel>
      <Header>
        ...
        <div key="modalContextHolder">{modalContextHolder}</div>
      </Header>
