...
const Chart404Graph: FC<{
  ...
  const renderIcon = (iconStr: any) => {
    if (/^<svg/.test(iconStr) || /^<\?xml/.test(iconStr)) {
      return <StyledImageIcon src={`data:image/svg+xml;utf8,${iconStr}`} />;
    }
    if (/svg\+xml;base64/.test(iconStr)) {
      return <StyledImageIcon src={iconStr} />;
    }
    return (
      <StyledChartIcon>
        <i className={`iconfont icon-${!iconStr ? 'chart' : iconStr}`}></i>
      </StyledChartIcon>
    );
  };
