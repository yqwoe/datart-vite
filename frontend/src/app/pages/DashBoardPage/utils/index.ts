  ...
  controllerWidgets.forEach(filterWidget => {
    ...
    if (
      ...
      if (Array.isArray(relatedViewItem.fieldValue)) {
        const key1 = String(relatedViewItem.fieldValue?.[0]);
        const key2 = String(relatedViewItem.fieldValue?.[1]);
...
export const getControllerDateValues = (obj: {
  ...
  const timeValues: [string, string] = ['', ''];
...
export const adjustRangeDataEndValue = (
  ...
  const end = adjustTime.format(TIME_FORMATTER);
  return end;
};
export const getChartWidgetRequestParams = (obj: {
  ...
  const requestParams = getDataChartRequestParams({
    dataChart,
    view: chartDataView,
    option: option,
    drillOption,
