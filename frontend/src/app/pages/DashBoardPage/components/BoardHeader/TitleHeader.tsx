...
import { EditOutlined, SendOutlined, SettingOutlined } from '@ant-design/icons';
...
export const TitleHeader: FC = memo(() => {
  ...
  const toBoardEditor = () => {
    ...
    if (pathName.includes(boardId)) {
      history(`${pathName.split(boardId)[0]}${boardId}/boardEditor`);
    } else if (pathName.includes('/vizs')) {
      history(`${pathName.split('/vizs')[0]}${'/vizs/'}${boardId}/boardEditor`);
    }
  };

  const saveToStoryOk = useCallback(
    (storyId: string) => {
      history(`/organizations/${orgId}/vizs/storyEditor/${storyId}`, {
        state: {
