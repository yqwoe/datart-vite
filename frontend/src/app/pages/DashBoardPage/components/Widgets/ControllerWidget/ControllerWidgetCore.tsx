...
export const ControllerWidgetCore: React.FC<{}> = memo(() => {
  ...
  const control = useMemo(() => {
    const selectOptions = optionRows?.map(ele => {
    ...
    switch (facadeType) {
      ...
      case ControllerFacadeTypes.Slider:
        ...
        const minValue = config.minValue === 0 ? 0 : config.minValue || 1;
        const maxValue = config.maxValue === 0 ? 0 : config.maxValue || 100;
        return (
          <SlideControllerForm
            onChange={onControllerChange}
            minValue={minValue}
      ...
      case ControllerFacadeTypes.RadioGroup:
        form.setFieldsValue({ value: controllerValues?.[0] });
        const RadioOptions = optionRows?.map(ele => {
          return { value: ele.key, label: ele.label } as ControlOption;
        });
        const radioButtonType = config.radioButtonType;
        return (
      ...
      case ControllerFacadeTypes.RangeTime:
        ...
        const rangePickerType = controllerDate!.pickerType;
        return (
      ...
      case ControllerFacadeTypes.Time:
        ...
        const pickerType = controllerDate!.pickerType;
...
const Wrapper = styled(({ formFrontColor, ...props }) => <div {...props} />)<{
  formFrontColor: string;
}>`
  display: flex;
  flex: 1;
  align-items: center;
