  ...
  (dispatch, getState) => {
    ...
    if (jumpConfig?.targetType === 'INTERNAL') {
      ...
      if (typeof jumpConfig?.filter === 'object') {
        ...
        history(`/organizations/${orgId}/vizs/${targetId}?${searchParamsStr}`);
      }
    }
  };
...
export const widgetLinkEventAction =
  ...
  async (dispatch, getState) => {
    ...
    boardLinkWidgets.forEach(targetWidget => {
      const dataChart =
        dataChartMap[targetWidget.datachartId]?.[targetWidget.datachartId];
...
export const setWidgetSampleDataAction =
  (args: { boardId: string;boardEditing: boolean; datachartId: string; wid: string }) =>
  (dispatch, getState) => {
