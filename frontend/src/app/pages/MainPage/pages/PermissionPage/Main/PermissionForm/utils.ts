  ...
  switch (type) {
    case ResourceTypes.Viz:
      return type === ResourceTypes.Viz
        ...
        : id === type;
    case ResourceTypes.Extract:
      return type === ResourceTypes.Extract
        ? Object.values(ExtractResourceSubTypes).includes(
            id as ExtractResourceSubTypes,
          )
        : id === type;
  }
}
...
export function parsePermission(
  ...
  const permissionViewModel: PermissionLevels[] = [];
...
export function getExtraPrivilege(value) {
  const extraValue: PermissionLevels[] = [];
