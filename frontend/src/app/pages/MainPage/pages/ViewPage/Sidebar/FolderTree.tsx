...
export const FolderTree = memo(({ treeData, searchable }: FolderTreeProps) => {
  ...
  useEffect(() => {
    if (searchable && treeData?.length) {
      setExpandedKeys(expandedKeys.concat(treeToArray(treeData).map(({ key }) => key)));
    }
  }, [searchable, treeData]);
  ...
  const redirect = useCallback(
    currentEditingViewKey => {
      if (currentEditingViewKey) {
        history(`/organizations/${orgId}/views/${currentEditingViewKey}`);
      } else {
        history(`/organizations/${orgId}/views`);
      }
    },
    [history, orgId],
  );
  ...
  const treeSelect = useCallback(
    (_, { node }) => {
      ...
      if (!node.isFolder && node.id !== currentEditingViewKey) {
        history(`/organizations/${orgId}/views/${node.id}`);
      }
    },
