...
import { Button, Checkbox, Divider, Empty, Input, Menu, Popover, Tooltip } from 'antd';
...
const SelectDataSource = memo(
  ({
    ...
    const treeTitleRender = useCallback(
      nodeData => {
        if(nodeData?.comment){
          return <Tooltip title={nodeData?.comment} placement='right' arrow>
            {nodeData?.title}
          </Tooltip>
        }
        return (
          ...
          content={
            currentSources ? (
              ...
              <PopoverBody>
                ...
                <SourceList>
                  <Menu onClick={handleCurrentSources}>
