...
import { Category, CATEGORY_LABEL } from '../constants';
...
export function SourceDetailPage() {
  ...
  const [depens, setDepens] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (editingSource) {
      const { name, category, type, config } = editingSource;
      try {
        setProviderType(type);
        const configObj = JSON.parse(config);
        setDepens(configObj?.depens || []);
        setExtract(category === Category.EXTRACT);
  ...
  useEffect(() => {
    const resetTemplate = setTimeout(() => {
      providerType && dispatch(getDataProviderConfigTemplate(providerType));
    }, 50);
    return () => clearTimeout(resetTemplate);
  }, [dispatch, providerType, tg, t]);
  ...
  const dbTypeChange = useCallback(
    val => {
      ...
      if (dbTypeConfig) {
        ...
        if (selected) {
          const { url, driverClass, db } = selected;
          form.setFieldsValue({ config: { url, driverClass, db } });
        }
      }
    },
    [config, form],
              ...
              dispatch(
                addSource({
                  ...
                  resolve: (id: string) => {
                    ...
                    history(`/organizations/${orgId}/sources/${id}`);
                    onClose();
                  },
                }),
              );
      ...
      dispatch(
        deleteSource({
          ...
          resolve: () => {
            ...
            history(`/organizations/${orgId}/sources`);
          },
        }),
      );
    },
  ...
  const unarchive = useCallback(() => {
    ...
    showSaveForm({
      ...
      onSave: (values, onClose) => {
        const index = getInsertedNodeIndex(values, sourceData);
        dispatch(
          unarchiveSource({
            source: {
              name: values.name,
  ...
  const categoryChange = useCallback(
    value => {
      switch (value) {
        case Category.ANALYZE:
          setExtract(false);
          break;
        case Category.EXTRACT:
          setExtract(true);
          break;
      }
    },
  ...
  const onDependencyChange = useCallback(val => {
    setDepens([...depens, val]);
  }, []);
