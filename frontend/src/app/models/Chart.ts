...
import ChartDataConfigSection from 'app/pages/ChartWorkbenchPage/components/ChartOperationPanel/components/ChartDataConfigSection';
import {
  ChartMouseEvent,
  ChartStatus,
  IChart,
    ...
    return (current || [])
      ...
      .every(cc => {
        ...
        if (tc?.length > 1) {
          ...
          if (!subTc) {
            const subTcTotalLength = tc.flatMap(tc => tc.rows)?.filter(Boolean)
              ?.length;
            return isInRange(
              cc?.limit,
