...
const areas = import.meta.glob('../../../assets/geojson/areas/**.json');
const cities = import.meta.glob('../../../assets/geojson/cities/**.json');
...
const provinces = geoChina?.features?.map?.(f => f.properties.adcode);
const citites = Object.keys(areas).map(city =>
  city.replace('../../../assets/geojson/areas/', '').replace('.json', ''),
);
const provinceCodeMap = {};
geoChina?.features?.forEach(f => {
  provinceCodeMap[f.properties.name] = String(f.properties.adcode);
});
...
provinces.forEach(async province => {
  const loadFile = cities[`../../../assets/geojson/cities/${province}.json`];
  if (loadFile) {
    ...
    registerMap(String(province), json as any);
    provinceMap[province] = json;
  }
});

citites.forEach(async city => {
  const loadFile = areas[`../../../assets/geojson/areas/${city}.json`];
  if (loadFile) {
    const res = await loadFile();
    const json = {
      UTF8Encoding: true,
    ...
    registerMap(String(city), json as any);
    provinceMap[city] = json;
  }
});
...
class BasicOutlineMapChart extends Chart {
  ...
  private getOptions(
    ...
    this.registerGeoMap(styleConfigs, drillOption);
  ...
  private getMapLevelName(
    styleConfigs: ChartStyleConfig[],
    drillOption?: ChartDrillOption,
  ) {
    const [mapLevelName] = getStyles(styleConfigs, ['map'], ['level']);
    let levelName = mapLevelName;
    const conditons = drillOption
      ?.getAllDrillDownFields()
      .filter(field => Boolean(field.condition));
    const last = conditons?.[conditons.length - 1];
    if (last && last?.condition) {
      levelName = provinceCodeMap[last?.condition?.value];
    }

    return levelName;
  }

  private registerGeoMap(
    styleConfigs: ChartStyleConfig[],
    drillOption?: ChartDrillOption,
  ) {
    const levelName = this.getMapLevelName(styleConfigs, drillOption);
    this.geoMap = provinceMap[levelName];
  }
  ...
  private getGeoInfo(
    ...
    const [areaColor, areaEmphasisColor, enableFocus, borderStyle] = getStyles(
      styleConfigs,
      ['map'],
      ['areaColor', 'areaEmphasisColor', 'focusArea', 'borderStyle'],
    );

    const [top, bottom, left, right] = getStyles(
      styleConfigs,
      ['margin'],
    ...
    return [
      {
        ...
        emphasis: {
          label: {
            show: showLabel,
            textStyle: {
              fontSize: Number(font?.fontSize) + 20,
            },
          },
          itemStyle: {
            color: areaEmphasisColor,
    ...
    const targetName = this.geoMap.features.find(f =>
      isProvinceLevel
        ? f.properties.name?.includes(sourceName) ||
          sourceName?.includes(f.properties.name)
  ...
  protected mappingGeoCoordination(
    ...
    return (
      (properties?.cp || properties?.center || properties?.centroid)?.concat(
        values,
      ) || []
    );
  }
  ...
  protected getVisualMap(
    ...
    const inRange = {
      color: [
        aggregateConfigs?.[0]?.color?.start || '#8696A7',
