...
const ChartDrillContextMenu: FC<{
  ...
  const selectDrillStatusMenu = useMemo(() => {
    return (
      <Menu.Item key="selectDrillStatus" icon={<CheckOutlined />}>
        <StyledMenuSwitch
    ...
    return (
      ...
      >
        ...
        {onViewDataChange && hasViewDetailSetting && (
          <Menu.Item key={'viewData'} icon={<ArrowDownOutlined />}>
            {t('viewData')}
          </Menu.Item>
        )}
        {drillOption && drillOption?.getCurrentDrillLevel() > 0 && (
          <Menu.Item key={'rollUp'} icon={<ArrowUpOutlined />}>
            {t('rollUp')}
          </Menu.Item>
        )}
        {drillOption &&
          ...
          !drillOption?.isBottomLevel && (
            <Menu.Item key={DrillMode.Drill} icon={<ArrowDownOutlined />}>
              {t('showNextLevel')}
            </Menu.Item>
          )}
        {drillOption &&
          ...
          !drillOption?.isBottomLevel && (
            <Menu.Item key={DrillMode.Expand} icon={<ArrowDownOutlined />}>
              {t('expandNextLevel')}
            </Menu.Item>
          )}
        {drillOption &&
  ...
  if (!drillOption?.isDrillable) return children;
